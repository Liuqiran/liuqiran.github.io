---
title: 🧭 What I'm Doing Now
translationKey: now
slug: "now"
author: Qiran
type: page
cover:  
  image: "/img/clock-wall.jpg"   
showToc: false   
ai_assisted: true
---
> This is what I’m currently focused on. Inspired by [nownownow.com](https://nownownow.com/about).
>
<!--<div style="clear: both;">-->
<!--<iframe src="https://twisty-change-610.notion.site/ebd/1fc21e6176cd8080be08ea1b2214e8b3?v=1fc21e6176cd813b966e000c20634f61" width="100%" height="600" frameborder="0" allowfullscreen></iframe>-->   
<!--</div>-->  

# 🖥️ Plans   

- [ ] Fix the URL of Waline system 
- [ ] Digitalize 日记③ 
- [ ] Digitalize 日记④  
- [ ] Create default images for posts 
- [x] Fix Hugo backend comment URLs
- [x] Optimize word count statistics
- [x] Add email subscription feature   
- [X] Setting level system for Waline  
- [X] Change social buttons of home pages
- [x] Add image and statement at the end of the post (different in each language)
- [ ] Add Substack button  
- [ ] ~~Open Gumroad/Buy me a coffee account~~ 
- [ ] ~~Add feature of Archive page ( Can find posts  by year/month)~~ 


# ✒️ [Moments](/en/tags/journal/)      



# 🌳 Building Habits

## How My Current Habit System Is Going

*Updated July 17 2026.*

Building and maintaining a stable habit system is the most important practical foundation of my life. It is how I try to turn my values, intentions, and long-term goals into repeated action.   

I currently track **37** active habits. They range from small daily maintenance routines to long-term practices that shape how I live. Some of the habits that matter most to me include:

- Maintaining a consistent sleep schedule;
- Reading and studying regularly;
- Journaling;
- Exercising;

![Full history of my weighted habit-strength score](./habit-strength-history.svg)

The rightmost part of the chart represents my most recent habit strength. A rising line generally means that I have been following my current habits more consistently. A falling line means that my routines have recently become less stable.

This is not a score of my happiness, success, morality, productivity, or value as a person. It measures the consistency of the system through which I try to live according to what matters most to me:

> How consistently am I practising the habits I currently intend to maintain?

<details>
<summary><strong>How this score works</strong></summary>

Each habit receives a score using a Loop-style habit-strength algorithm. Repeated completion gradually strengthens the score, while missed scheduled opportunities weaken it. Recent behaviour has a greater influence than distant behaviour, although older records remain part of the calculation.

The overall score is a weighted average of the individual habit scores. Habits placed higher in my current tracking order receive slightly greater weight.

The historical line is calculated from the habits that are active in my current system, using the available records since each habit began. For longer time spans, daily scores may be grouped into weekly or monthly averages so that the chart remains readable.

Because I may add, archive, reorder, or redefine habits over time, this score is best understood as an evolving personal indicator rather than a fixed scientific measurement.

</details>


